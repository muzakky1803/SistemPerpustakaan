...
class Kategori extends CI_Controller {
	...
	public function loadTable()
	{
		...
		if($queryData <> FALSE) {
			...
			foreach($queryData->result() as $query) {
				if($query->id_kategori > 0) {
					$result['data'][] = [
						$no,
						$query->keterangan,
						
						'
						<button onclick="showData('.$query->id_kategori.')" class="btn btn-warning btn-circle btn-sm"><i class="fas fa-edit"></i></button>
						<button onclick="deleted('.$query->id_kategori.')" class="btn btn-danger btn-circle btn-sm" title="Delete Data"><i class="fa fa-trash"></i></button>
						'
					];
					$no++;
				}
	...
	public function showData($id)
	{
		...
		$data  = $this->query->getData('*', 'kategori_buku', $where)->row();
	...
	public function updateData($id)
	{
		...
		$table  = ['column' => 'id_kategori', 'param' => $id, 'table' => 'kategori_buku'];
